...
function get_k() {
  ...
  i_k =
    ((-Number(k_input.value) % ALPHABET_SIZE) + ALPHABET_SIZE) % ALPHABET_SIZE;
  i_k_input.value = i_k;
}
...
function encode() {
  ...
  for (var i = 0; i < txt.length; i++) {
    ...
    if (c <= 90 && c >= 64) {
      d =
        ((((c - 64 + k) % ALPHABET_SIZE) + ALPHABET_SIZE) % ALPHABET_SIZE) + 64;
    } else {
      d = c;
    }
    end_sequence.push(d);
...
function decode() {
  ...
  for (var i = 0; i < txt.length; i++) {
    ...
    if (c <= 90 && c >= 64) {
      d =
        ((((c - 64 + i_k) % ALPHABET_SIZE) + ALPHABET_SIZE) % ALPHABET_SIZE) +
        64;
    } else {
      d = c;
    }
